variable "vpc_config" {
  type = object({
    vpc_primary_cidr          = string
    vpc_secondary_cidr        = string
    az_count                  = number
  ...
  validation {
    condition     = can(cidrhost(var.vpc_config.vpc_primary_cidr, 32))
    error_message = "Input variable vpc_config.vpc_primary_cidr must be a valid IPv4 CIDR."
  }
  validation {
    condition     = can(cidrhost(var.vpc_config.vpc_secondary_cidr, 32))
    error_message = "Input variable vpc_config.vpc_secondary_cidr must be a valid IPv4 CIDR."
  }
  validation {
